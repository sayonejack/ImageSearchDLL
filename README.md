...
### **2. ImageSearch_XP.dll (Legacy Version)**
...
1. **Prioritize External DLL:** When the _ImageSearch function is called, the UDF first looks for ImageSearch_x86.dll and ImageSearch_x64.dll in the same directory as the script (@ScriptDir). If found, it uses this file to achieve the best performance (with AVX2 if available).  
...
➡️ **This ensures that your script can always run**, even if you forget to copy the DLL file. However, for the highest speed, always place the modern ImageSearch_x86.dll and ImageSearch_x64.dll next to your script.

## **📦 Setup**

1. **Place the DLL file:** Copy ImageSearch_x86.dll and ImageSearch_x64.dll (the modern version) into the same directory as your AutoIt script file. 
2. **Include the UDF in your script:** Use the line #include <ImageSearch_UDF.au3> in your script.
...
; Example output: {0}[No Match Found] | DEBUG: File=image_not_exist.png, Rect=(0,0,1920,1080), Tol=10, Trans=0xffffffff, Multi=0, Center=1, FindAll=0, AVX2=true, Scale=(1.00,1.00,0.10)
```
